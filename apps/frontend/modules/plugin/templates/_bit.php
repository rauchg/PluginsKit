...
<li>
	<a href="<?php echo url_for('plugin', array('slug' => $plugin->getSlug())) ?>">
		<span class="name"><?php echo highlight_text($plugin->getTitle(), isset($search) ? $search : '') ?> <?php if ($plugin->isOfficial()) echo image_tag('/images/icon.png', 'alt=Official plugin title=Official plugin') ?></span>
