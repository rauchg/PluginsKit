...
var ReMooz = new Class({
	...
	close: function() {
		...
		this.box.set('tween', {duration: 200}).fade(0);
		return this;
	},
...
var Loaders = new Class({
  
  addLoadersSync: function(name, obj, sync) {
    this.$syncedLoaders = this.$syncedLoaders || $H();
    this.$syncedLoaders.set(name, obj);
    if(! sync) obj.addLoadersSync(name, this, true);
    return this;
  },
  
  removeLoadersSync: function(name, sync) {
    if(! this.$syncedLoaders) return this;
    var obj = this.$syncedLoaders.get(name);
    this.$syncedLoaders.erase(name);
    if(! sync) obj.removeLoadersSync(name, true);
    return this;
  },
  
  addLoader: function(name) {
    this.$loaders = this.$loaders || $H();
    if(! this.isLoading()) this.startLoading();
    this.$loaders.set(name || '$', 1);
    return this;
  },
  
  removeLoader: function(name) {
    if(! this.$loaders) return this;
    this.$loaders.erase(name || '$');
    if(! this.isLoading()) this.stopLoading();
    return this;
  },
  
  isLoading: function(synced) {
    return !! this.$loaders.getLength() && (this.$syncedLoaders ? this.$syncedLoaders.some(function(obj) { return obj != synced ? obj.isLoading(this) : false }) : true);
  }
		...
		if ($('add-plugin-form')){
			var request = new ForgeGitHubRequest({
				...
				onFirstRequest: function(){
					$('plugin_add_submit').addClass('input_submit_disabled');
				},
